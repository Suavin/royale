...
vRPclient = Tunnel.getInterface("vRP","j_anticheat")

--MySQL = module("vrp_mysql", "MySQL")
--MySQL.createCommand("vRP/HG_Ban", "UPDATE vrp_users SET banned='1' WHERE id = @id")

RegisterServerEvent("j_anticheat:Cars")
AddEventHandler("j_anticheat:Cars", function()
	local user_id = vRP.getUserId({source})
	local player = vRP.getUserSource({user_id})
	local name = GetPlayerName(source)
	print("[j_anticheat] | " ..name.. "["..user_id.. "] A PRIMIT BAN (CARS BLACKLISTED)!")
	TriggerClientEvent('chatMessage', -1, '^3[j_anticheat]', {255, 0, 0}, "^1" ..name.. "^3[ID:" ..user_id.. "]^1 A PRIMIT BAN ^3(reason: CARS BLACKLISTED)!" )
	DropPlayer(source, "[j_anticheat] | AI FOST DETECTAT CU HACK! (CARS BLACKLISTED)")
...
RegisterCommand("ac", function(thePlayer, args, rawCommand)
	TriggerClientEvent("j_anticheat:Toggle", -1, 1)
end)
