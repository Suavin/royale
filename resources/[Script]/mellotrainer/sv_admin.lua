...
AddEventHandler( 'mellotrainer:adminTempBan', function( id ) 
	...
	if ( license ~= nil ) then 
		...
		DATASAVE:print( GetPlayerName( id ) .. " foi temporariamente proibido por " .. GetPlayerName( source ) .. "." )
		DropPlayer( id, "Banido: Você foi temporariamente banido." )
	end 
end )
...
AddEventHandler( 'playerConnecting', function( name, setReason ) 
	...
	if ( license ~= nil ) then 
		if ( tempBannedUsers[license] ) then 
			DATASAVE:print( GetPlayerName( source ) .. " está temporariamente banido, recusando conexão." )
			setReason( "Banido: Você foi temporariamente banido." )
			CancelEvent()
		end 
	end 
end )
