...
local function SpawnVehicle(model, x, y, z, heading, ped)
	...
	if IsModelValid(model) then
		...
		drawNotification("[~p~Chicago~w~] ~g~Veiculo spawnado")
		lastVehicle = veh
		...
		drawNotification("[~p~Chicago~w~] ~r~Modelo invalido")
	end
end
...
RegisterNUICallback("vehspawnoptions", function(data,cb)
	...
	if data.action == "despawn" then
		...
		drawNotification("[~p~Chicago~w~] Deletar veiculo anterior: "..tostring(text))
	elseif data.action == "insidecar" then
		featureSpawnInsideCar = data.newstate
		drawNotification("[~p~Chicago~w~] Spawnar dentro do veiculo: "..tostring(text))
	elseif data.action == "infront" then 
		featureSpawnCarInFront = data.newstate
		drawNotification( "[~p~Chicago~w~] Spawnar veiculo na frente " .. tostring( text ) )
	end 
...
function CreateVehicleOptions( index )
	...
	local overwriteSave = {
		[ "menuName" ] = "Sobrescrever com corrente", 
		[ "data" ] = {
...
RegisterNUICallback( "loadsavedvehs", function( data, cb ) 
    ...
    table.insert( validOptions, {
    	[ "menuName" ] = "Criar novo save de veiculo", 
    	[ "data" ] = {
    		[ "action" ] = "vehiclesave"
    	}
    } )
    	...
    	if ( DoesEntityExist( ped ) and not IsEntityDead( ped ) ) then 
    		...
    		if ( GetPedInVehicleSeat( veh, -1 ) == ped ) then 
                while ( (saveName == nil and not overwriting) or (saveName == nil and renaming) ) do 
                    saveName = requestInput( "Entrar com nome do save", 24 )
                    Citizen.Wait( 1 )
                end 
...
RegisterNUICallback("vehcolor", function(data, cb)
	...
	if not playerVeh then
		drawNotification("[~p~Chicago~w~] ~r~Não em um veículo!")
		return
	end

	SetVehicleCustomPrimaryColour(playerVeh, r, g, b)
	SetVehicleCustomSecondaryColour(playerVeh, r, g, b)
	drawNotification("[~p~Chicago~w~] ~g~Veículo Repintado!")
...
RegisterNUICallback("requirevehicle", function(data, cb)
	...
	elseif(not(playerPed == (GetPedInVehicleSeat(playerVeh,-1))))then
		...
		if (vehClass == 14 or vehClass == 21) then
			...
			drawNotification("[~p~Chicago~w~] ~r~Este veículo não suporta modificações.")
		else
...
RegisterNUICallback("veh", function(data, cb)
	...
	if action == "toggleinstantly" then
		...
		drawNotification("[~p~Chicago~w~] Portas Abertas Instantaneamente: "..text)
		return
	elseif action == "speedometer" then
		featureSpeedometer = data.newstate
		drawNotification("[~p~Chicago~w~] Speedometer: "..text)
		return
	end
	...
	if playerVeh <= 0 then
		drawNotification("[~p~Chicago~w~] ~r~Não em um veiculo.")
		return
	end
	...
	elseif action == "fixset"then
		...
		drawNotification("[~p~Chicago~w~] ~g~Veículo reparado e definido corretamente.")

	elseif action == "fix" then
		SetVehicleFixed(playerVeh)
		drawNotification("[~p~Chicago~w~] ~g~Veículo reparado.")


	elseif action == "dirty" then
		SetVehicleDirtLevel(playerVeh, 15.0)
		drawNotification("[~p~Chicago~w~] ~g~Veículo sujo.")


	elseif action == "clean" then
		SetVehicleDirtLevel(playerVeh, 0.0)
		drawNotification("[~p~Chicago~w~] ~g~Veiculo limpo.")
...
RegisterNUICallback("vehmod", function(data, cb)
	...
	elseif action == "xenonlights" then
		...
		drawNotification("[~p~Chicago~w~] Xenon: "..text)
	...
	elseif action == "changewheeltype" then
		...
		drawNotification("[~p~Chicago~w~] ~g~Categoria de roda alterado")
		local data = createVariableData()
...
RegisterNUICallback("vehmodify", function(data, cb)
	...
	drawNotification("[~p~Chicago~w~] Modificação aplicada.")
end)
...
RegisterNUICallback("vehopts", function(data, cb)
	...
	if(action == "nodrag")then
		...
		drawNotification("[~p~Chicago~w~] Sem Arrasto: "..text)

	-- No Fall Off
	elseif(action == "nofall")then
		featureNoFallOff = state;
		featureNoFallOffUpdated = true;
		drawNotification("[~p~Chicago~w~] Nenhuma queda: "..text)

	-- No Helmet
	elseif(action == "nohelmet")then
		featureNoHelmet = state;
		drawNotification("[~p~Chicago~w~] Sem capacete: "..text)
	end


	if(not IsPedInAnyVehicle(playerPed, false))then
		drawNotification("[~p~Chicago~w~] Não em um veiculo.")
		return
	end

	local playerVeh = GetVehiclePedIsUsing(PlayerPedId())
	if(not(playerPed == (GetPedInVehicleSeat(playerVeh,-1))))then
		drawNotification("[~p~Chicago~w~] Não proprietário de veículo.")
		return
	end
	...
	if(action == "driftmode")then
		if(not(IsThisModelACar(GetEntityModel(playerVeh))))then
			drawNotification("[~p~Chicago~w~] Veículo deve ser um carro")
			return
		end

		featureDriftMode = state
		SetVehicleReduceGrip(playerVeh, featureDriftMode)
		drawNotification("[~p~Chicago~w~] Modo drift: "..text)

	-- Power Options
	elseif(action == "powerboost")then
		featurePowerMultiplier = tonumber(data.data[3])

		drawNotification("[~p~Chicago~w~] Multiplicador de motor: "..tostring(featurePowerMultiplier))

	-- Torque Options
	elseif(action == "torqueboost")then
		featureTorqueMultiplier = tonumber(data.data[3])

		drawNotification("[~p~Chicago~w~] Multiplicador de torque: "..tostring(featureTorqueMultiplier))
