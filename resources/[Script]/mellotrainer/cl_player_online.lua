...
function spectatePlayer(target)
	...
	if(featureSpectate)then
		...
		drawNotification("[~p~Chicago~w~] Assistindo ~b~<C>"..target['menuName'].."</C>.")
	else
		if(not IsScreenFadedOut() and not IsScreenFadingOut()) then
			DoScreenFadeOut(1000)
			while (not IsScreenFadedOut()) do
		...
		drawNotification("[~p~Chicago~w~] Você parou de assistir ~b~<C>"..target['menuName'].."</C>.")
	end
end
...
function drawRoute(target)
	...
	if (featureDrawRoute) then
		if (DoesEntityExist(target['ped'])) then
			...
			drawNotification("[~p~Chicago~w~] Drawing Live Route To:~n~~b~<C>"..target['menuName'].."</C>.")
		end
	else
		SetWaypointOff()
		drawNotification("[~p~Chicago~w~] Route Removed.")
	end
end
...
function updateDrawRoute()
	...
	if(featureDrawRoute)then
		if(NetworkIsPlayerConnected(targetId))then
			...
			featureDrawRoute = false
			drawRouteTarget = nil
			drawNotification("[~p~Chicago~w~] Usuário "..target['menuName'].."~r~<C>Desconectou</C>.")
		end
	end
end
...
function teleportToPlayer(target)
	...
	drawNotification("[~p~Chicago~w~] Teleportado para ~b~<C>"..target['menuName'].."</C>.")
end
...
function teleportIntoPlayerVehicle(target)
	...
	if(not IsPedInAnyVehicle(target['ped'], false))then
		drawNotification("[~p~Chicago~w~] ~b~<C>"..target['menuName'].."</C> ~s~não está em nenhum veículo")
		return
	end
	...
	if(targetVeh == GetVehiclePedIsIn(playerPed))then
		drawNotification("[~p~Chicago~w~] Você já está em ".."~b~<C>"..target['menuName'].."'s</C> ~s~Veiculo")
		return
	end

	if(GetVehicleDoorsLockedForPlayer(targetVeh, playerPed))then
		drawNotification("[~p~Chicago~w~] ~b~<C>"..target['menuName'].."'s</C> ~s~Veiculo trancado.")
		return
	end
	...
	if (seatNum >= passNum) then
		drawNotification("[~p~Chicago~w~] ~b~<C>"..target['menuName'].."'s</C> ~s~Veiculo cheio")
	    return
	end

	drawNotification("[~p~Chicago~w~] Teleportado para ~b~<C>"..target['menuName'].."</C> ~s~Veiculo.")
end
...
function toggleRelationshipBlip(colorID, typeText, target)
	...
	drawNotification("[~p~Chicago~w~] Marcou ~b~<C>"..target['menuName'].."</C>~s~ como "..typeText)
end
...
RegisterNUICallback("otherplayer", function(data, cb)
	...
	if(target == nil)then
		drawNotification("[~p~Chicago~w~] O usuário ~r~<C>Desconectou</C>.")
		return
	end
	...
	if action == "relationship" then
		...
		if(relationshipType == "friendly")then
			toggleRelationshipBlip(3, "Amigo",target)
		...
		elseif(relationshipType == "hostile")then
			toggleRelationshipBlip(1, "Hostil",target)
		end
...
RegisterNUICallback("getonlineplayers", function(data,cb)
	...
	if(playerCount < 1)then
		drawNotification("[~p~Chicago~w~] ~r~Sem player em sua sessão.")
		return
	end
