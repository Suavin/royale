...
function GeneratePlayerAdminMenus( id )
	...
	local kick_reason = {
		[ "menuName" ] = "Kick (Espeficicar a rasão)", 
		[ "data" ] = {
...
RegisterNUICallback( "adminkick", function( data, cb ) 
	if ( data.action == "input" ) then 
		...
		if ( reason ) then 
			TriggerServerEvent( 'mellotrainer:adminKick', id, "Kikado: " .. reason )
		end 
	else 
		local id = tonumber( data.action )

		TriggerServerEvent( 'mellotrainer:adminKick', id, "Você foi removido do servidor" )
	end 
end )
...
RegisterNUICallback("time", function(data, cb)
	...
	if (action == "input") then
		...
		if (request == nil) or (request > 24) or (request < 0) then
			drawNotification("~r~Você deve especificar um número de 0 a 24.")
			return
		else
			h=request
			m=0
	...
	drawNotification("~g~Tempo mudado para "..tostring(h)..":"..string.format("%02d", tostring(m)))
end)
...
RegisterNUICallback("weatheroptions", function(data, cb)
	...
	elseif(action == "freeze")then
		...
		drawNotification("Tempo persistente "..text.."~w~, Selecione o tempo para aplicar")
...
RegisterNUICallback("weather", function(data, cb)
	...
	drawNotification("Clima alterado para~g~" .. weather .. ".")
	cb("ok")
end)
...
AddEventHandler("mellotrainer:adminstatus", function(status)
	if(status)then
		...
		drawNotification("~r~Você não é um administrador!")
	end		
end)
