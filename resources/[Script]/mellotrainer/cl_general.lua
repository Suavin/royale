...
function teleportToWaypoint()
	...
	if(not IsWaypointActive())then
		drawNotification("[~p~Chicago~w~] ~r~Sem marcação no mapa.")
		return
	end
	...
	drawNotification("[~p~Chicago~w~]Teleportado")
end
...
RegisterNUICallback("savefeaturevariables", function()
	...
	drawNotification("[~p~Chicago~w~] Configurações salvas")
end)

RegisterNUICallback("resetfeaturevariables", function()
	TriggerServerEvent( "wk:DataSave", "toggles", {}, 0)
	drawNotification("[~p~Chicago~w~] Configurações salvas apagadas Reconectar para ressincronizar")
end)
...
AddEventHandler("wk:RecieveSavedToggles", function(data)
	...
	syncSettings()
	drawNotification("[~p~Chicago~w~] Configurações carregadas")
end)