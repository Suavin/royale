...
local propComponents = {
	{ name = "Chapéus / Máscara / Capacetes", t = 0},
	{ name = "Óculos", t = 1},
	{ name = "Orelhas / Acessórios", t = 2}
}
	...
	Citizen.CreateThread( function()
		...
		if ( DoesEntityExist( ped ) and not IsEntityDead( ped ) ) then 
			while ( (saveName == nil and not overwriting) or (saveName == nil and renaming) ) do 
                saveName = requestInput( "Digite o nome para salvar", 24 )
                Citizen.Wait( 1 )
            end 
...
RegisterNUICallback("playerskin", function(data, cb)
	...
	if(not(IsModelValid(model)))then
		drawNotification("[~p~Chicago~w~] ~r~Nome do modelo inválido")
		return
	end
