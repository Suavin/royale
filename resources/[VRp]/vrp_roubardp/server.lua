...
func = {}
Tunnel.bindInterface("vrp_roubardp",func)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local timers = 0
...
function func.checkRobbery(id,x,y,z,head)
	...
	if user_id then
		...
		elseif (os.time()-timers) <= 1800 then
			...
			recompensa = parseInt(math.random(55000,115000)/caixas[id].seconds)
			...
			for l,w in pairs(policia) do
				...
				if player then
					async(function()
						...
						TriggerClientEvent('chatMessage',player,"911",{65,130,255},"O roubo começou na ^1Delegacia^0, dirija-se até o local e intercepte os assaltantes.")
					end)
				end
			end
