...
RegisterCommand('toogle',function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"toogle.perm") then
		TriggerEvent('eblips:remove',source)
		vRP.addUserGroup(user_id,"PaisanaPolicia")
		TriggerClientEvent("Notify",source,"sucesso","Você saiu de serviço.")
	elseif vRP.hasPermission(user_id,"tooglen.perm") then
		TriggerEvent('eblips:remove',source)
		vRP.addUserGroup(user_id,"PaisanaPoliciaN")
		TriggerClientEvent("Notify",source,"sucesso","Você saiu de serviço.")
	elseif vRP.hasPermission(user_id,"comando.permissao") then
		TriggerEvent('eblips:remove',source)
		vRP.addUserGroup(user_id,"PaisanaPoliciaComando")
		TriggerClientEvent("Notify",source,"sucesso","Você saiu de serviço.")
	...
	elseif vRP.hasPermission(user_id,"paisanapolician.permissao") then
		TriggerEvent('eblips:add',{ name = "Policial Norte", src = source, color = 49 })
		vRP.addUserGroup(user_id,"PoliciaNorte")
		TriggerClientEvent("Notify",source,"sucesso","Você entrou em serviço.")
	elseif vRP.hasPermission(user_id,"paisanapoliciacomando.permissao") then
		TriggerEvent('eblips:add',{ name = "Comando", src = source, color = 30 })
...
RegisterCommand('anuncio',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"admin.permissao") then
		...
		vRPclient.setDiv(-1,"anuncio",".div_anuncio { background: rgba(255,50,50,0.8); font-size: 11px; font-family: arial; color: #fff; padding: 20px; bottom: 10%; right: 5%; max-width: 500px; position: absolute; -webkit-border-radius: 5px; } bold { font-size: 16px; }","<bold>"..mensagem.."</bold><br><br>Mensagem enviada por: ")
