...
function func.checkRobbery(id,x,y,z,head)
	...
	if user_id then
		...
		elseif (os.time()-timers) <= 32200 then
			TriggerClientEvent("Notify",source,"aviso","Os cofres estão vazios, aguarde <b>"..vRP.format(parseInt((12200-(os.time()-timers)))).." segundos</b> até que os civis depositem dinheiro.")
		else
			andamento = true
			timers = os.time()
			dinheirosujo = {}
			...
			recompensa = parseInt(math.random(1650000,2515000)/caixas[id].seconds)
			...
			for l,w in pairs(policia) do
				...
				if player then
					async(function()
						...
						TriggerClientEvent('chatMessage',player,"911",{65,130,255},"O roubo começou no ^1Banco Central^0, dirija-se até o local e intercepte os assaltantes.")
					end)
				end
			end
