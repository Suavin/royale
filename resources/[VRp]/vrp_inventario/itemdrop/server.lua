...
AddEventHandler('DropSystem:create', function(bags, item, count,Bagcoords)
    bag[bags] = {item = item, count = count}
    local itemdrop = vRP.getItemName(item)
    TriggerClientEvent('DropSystem:createForAll', -1, bags,Bagcoords,itemdrop)
end)
...
AddEventHandler('DropSystem:drop', function(bag, item, count)
    ...
    local itemdrop = vRP.getItemName(item)
    TriggerClientEvent('DropSystem:createForAll', -1, bag,itemdrop)
end)
