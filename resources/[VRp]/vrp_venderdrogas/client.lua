...
local rejected = false
    ...
    drawTxt(0.90, 1.40, 1.1,1.1,0.4, "~h~PRESSIONE ~r~H~w~ PARA VENDER DROGAS", 255, 255, 255, 255)
	...
	TriggerServerEvent('drogas:items')
    end
    end
    end
    end
    ...
    local player = GetPlayerPed(-1)
    local playerloc = GetEntityCoords(player, 0)
    drawTxt(0.90, 1.40, 1.0,1.0,0.4, "NEGOCIANDO ~r~" .. secondsRemaining .. "~w~ SEGUNDOS FALTANDO ~w~ ", 255, 255, 255, 255)
    local distance = GetDistanceBetweenCoords(pos1.x, pos1.y, pos1.z, playerloc['x'], playerloc['y'], playerloc['z'], true)

    if distance > 7 then
    exports.pNotify:SetQueueMax("left", 1)
    exports.pNotify:SendNotification({
    text = "Cancelado voce esta muito longe.",
    type = "error",
    ...
    if rejected then
    drawTxt(0.90, 1.40, 1.0,1.0,0.4, "SUA OFERTA FOI ~r~REJEITADA", 255, 255, 255, 255)
    end
    end
    end)
    ...
    Wait(0)
    if blah then
    TriggerServerEvent('drogas:dinheiro')
    blah = false
    selling = false
    end
    end
