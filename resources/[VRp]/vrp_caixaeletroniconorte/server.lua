...
func = {}
Tunnel.bindInterface("vrp_caixaeletroniconorte",func)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local timers = 0
...
function func.checkRobbery(id,x,y,z,head)
	...
	local policia = vRP.getUsersByPermission("tooglen.perm")
	if user_id then
		if #policia < 3 then
		...
		elseif (os.time()-timers) <= 1800 then
			...
			recompensa = parseInt(math.random(10000,25000)/caixas[id].seconds)
