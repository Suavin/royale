...
AddEventHandler("vehtuning",function(vehicle)
	...
	if IsEntityAVehicle(vehicle) then
		...
		SetVehicleNumberPlateText(vehicle,"ROYALE")
...
RegisterNetEvent('vrp_admin:godall')
AddEventHandler('vrp_admin:godall', function()
    for i , player in ipairs(GetActivePlayers()) do
        local ped = PlayerPedId()
        local otherPed = GetPlayerPed(player)
        if ped ~= otherPed then
            vRP.killGod(otherPed)
            vRP.varyHealth(otherPed,400)
        end
    end
end)

RegisterNetEvent('vrp_admin:tpall')
AddEventHandler('vrp_admin:tpall', function()
    for i, player in ipairs(GetActivePlayers()) do
        local ped = PlayerPedId()
        local otherPed = GetPlayerPed(player)
        local x,y,z = table.unpack(GetEntityCoords(ped))
        if ped ~= otherPed then
            vRP.teleport(otherPed,x,y,z)
        end
    end
end)