...
function vRP.setMochila(user_id, tMochila)
	...
	if vRP.getInventoryWeight(user_id) <= (vRP.getInventoryMaxWeight(user_id) - mochila[user_id] + tMochila) then
		...
		if tMochila == 0 then
			TriggerClientEvent('vrp_inventario:tiraMochila',source)
		end
		return true
	else 
		return false
...
function vRP.getInventoryMaxWeight(user_id)
	if not mochila[user_id] then mochila[user_id] = 0 end
	return math.floor(vRP.expToLevel(vRP.getExp(user_id, "physical", "strength")))*5 + mochila[user_id]
end
				...
				local function schest_enter(source)
					...
					if user_id and vRP.hasPermissions(user_id,schest.permissions or {}) then
						-- vRP.openChest2(source,"static:"..k,schest.weight or 0)
						 TriggerEvent("vrp_inventario:openChest","static:"..k,schest.weight or 0,user_id,user_id)
					end
				end
