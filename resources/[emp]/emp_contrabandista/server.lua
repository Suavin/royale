...
function emP.checkPayment()
	...
	if user_id then
		if vRP.tryGetInventoryItem(user_id,"wammo|WEAPON_ASSAULTSMG",quantidade[source]) then
			vRP.giveInventoryItem(user_id,"dinheirosujo",math.random(305,500)*quantidade[source])
		...
		for l,w in pairs(soldado) do
			...
			if player then
				async(function()
					...
					TriggerClientEvent('chatMessage',player,"911",{65,130,255},"Recebemos uma denuncia do tráfico de munições, verifique o ocorrido.")
