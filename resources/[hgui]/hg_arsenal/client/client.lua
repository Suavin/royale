		...
		for _,lugares in pairs(arsenal) do
			...
			if distance <= 2 then
				if IsControlJustPressed(0, 51) then
					TriggerServerEvent('hg_arsenal:permissao')
				end
			end
		end
	end
...
RegisterNetEvent('hg_arsenal:permissao')
...
RegisterNUICallback('Agua', function()
	...
	GiveWeaponToPed(ped,GetHashKey("WEAPON_VINTAGEPISTOL"),0,0,0)
end)
...
RegisterNUICallback('Conhaque', function()
	TriggerServerEvent('hg_arsenal:colete')
	print('a')
end)
...
RegisterNUICallback('HotDog', function()
	...
	GiveWeaponToPed(ped,GetHashKey("WEAPON_REVOLVER_MK2"),100,0,1)
end)
